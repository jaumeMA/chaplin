...
: function_base_t(ddk::detail::builtin_fusioned_function<ImSet,mpl::terse_function_dominion<DomSet>>(ddk::fusion(ddk::detail::instantiate_template_callable<typename ImSet::place_type>(i_callables,mpl::terse_function_dominion<DomSet>{})...)))
{
}
template<set_type ImSet,set_type DomSet>
...
{
    static_assert(IS_INDEXED_COND(ImSet),"You shall use coordinate target types in case of multi functions");
}
template<set_type ImSet,set_type DomSet>
...
template<set_type ImSet,set_type DomSet>
bool function<ImSet(DomSet)>::operator==(const function& other) const
{
    return false;
}
template<set_type ImSet,set_type DomSet>
