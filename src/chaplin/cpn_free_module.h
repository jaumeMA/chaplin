...
struct sub_basis_operation
{
    ...
    typedef ddk::high_order_array<ring_type,BasisOperation::rank() - ddk::mpl::get_num_ranks<SpecializedComponents...>()> set_traits;

    static constexpr size_t rank()
    {
        return BasisOperation::rank() - ddk::mpl::get_num_ranks<SpecializedComponents...>();
    }
    static inline set_traits basis(size_t i_index);
};
